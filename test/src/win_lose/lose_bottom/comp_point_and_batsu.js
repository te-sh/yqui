...
const setup = async t => {
  ...
  await t
    ...
    .click(s.dialog.rule.other.compPointAndPoint)
    .click(s.dialog.rule.submit)
    .click(s.subactions.master.allClear)
}

fixture('win_lose/lose_bottom/comp_point_and_batsu').beforeEach(setup).afterEach(closeWindows)
...
test('same comp point and different point', async t => {
  ...
  await t
    .click(s.subactions.master.loseBottom)
    .expect(s.box.players0.nth(0).find('.player-status').innerText).eql('Lose')
    .expect(s.box.players0.nth(1).find('.player-status').filterVisible().exists).notOk()
